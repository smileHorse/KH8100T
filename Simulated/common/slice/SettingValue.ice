...
*
* 文件名称：SettingValue.ice* 摘要：tdu100定值接口ice文件
*/


#ifndef _SETTINGVALUE_ICE
#define _SETTINGVALUE_ICE
...
module SettingValue{
	...
	const string SettingValueAppTopic = "settingvalue_app_operation";
	interface CSettingValueApp
	{
		void readValueReply(SettingValueInfo settingVal,bool flag,string tip); //读定值结果返回,,,flag：成功失败；tip：描述
		void writeValueReply(string deviceId,bool flag,string tip); //写定值结果返回
	};
	
	const string SettingValueFepTopic  = "settingvalue_fep_operation";
	interface CSettingValueFep
	{
		void readValue(string deviceId);//读定值
		void writeValue(SettingValueInfo settingVal);//写定值
	};

};

#endif	//_SETTINGVALUE_ICE